    ...
    <section className="max-w-screen-2xl mx-auto px-4 py-24">
      ...
      <article className="grid grid-cols-2 max-lg:grid-cols-1 gap-x-12 w-full mt-12">
        <div className="w-full rounded-lg overflow-hidden">
          <img alt="ecommerce" className="w-full object-cover aspect-video" src={urlForImage(data.sec2image).url()} />
        </div>

        <div className="mt-12 w-full">
          <h3 className="text-sm tracking-widest">{data.sec2title} <span className='text-golden'>{data.sec2date}</span></h3>
          <h2 className="font-medium text-4xl mt-3">{data.sec2heading}</h2>

          <p className="leading-relaxed mt-6">{data.sec2subheading}</p>
        </div>
      </article>
      ...
      <div className="grid grid-cols-3 max-md:grid-cols-1 justify-between mt-12 w-full gap-4">
        {data.sec3.map((item, i) => (
          <article key={i} className="p-4 w-full">
            ...
            <h3 className="text-sm tracking-widest pt-4">{item.sec3title} <span className='text-golden'>{item.sec3date}</span></h3>
